...
export function DeleteRolesFromUserForm() {
    ...
    async function DeleteRolesFromUser(values) {
        ...
        const response = await SendRequest('DeleteUserRolesRequest', request);
        ...
        NotifySuccess('success', `Successfully deleted user roles`);
    }
    ...
    return (
        <Card className="border-info border-3 bg-dark">
            <Card.Header className="text-warning border-3 border-info">Delete roles from user</Card.Header>
            <Card.Body>
                <Formik
                    initialValues={{...initialValues}}
                    onSubmit={async (values) => {
                        await DeleteRolesFromUser(values)
                    }}
                        ...
                        <Form className="row">
                            ...
                            <div className="col-6">
                                <button
                                    ...
                                    {state.isLoading ? <LoadingSpinner/> : 'Delete'}
                                </button>
                            </div>
